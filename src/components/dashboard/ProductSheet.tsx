...
import { Loader2, PackageX, RefreshCw, ShoppingCart, User as UserIcon, AlertCircle, ArrowRight } from "lucide-react";
        ...
        <div className="p-4 border-b bg-white/80 backdrop-blur-md sticky top-0 z-20">
          <SheetHeader>
            ...
            <div className="text-right">
              ...
              <SheetDescription className="text-xs">عرض المنتجات الحية وأسعار الشحن التلقائي</SheetDescription>
            </div>
          </SheetHeader>
        </div>
        ...
        <div className="flex-1 overflow-hidden bg-muted/30">
          {fetching ? (
            <div className="h-full flex flex-col items-center justify-center gap-3">
              ...
              <p className="text-sm font-bold text-muted-foreground animate-pulse">جاري تحديث الأسعار والمنتجات...</p>
            </div>
          ) : errorMsg ? (
              ...
              {filteredProducts.length > 0 ? (
                <div className="grid gap-4 pb-20">
                  {filteredProducts.map((product) => (
                    <Card key={product.id} className="border-none shadow-sm bg-white overflow-hidden group hover:ring-2 hover:ring-primary/20 transition-all">
                      <CardContent className="p-5 space-y-4">
                        ...
                        <div className="space-y-1.5">
                          ...
                          <Input 
                            placeholder="أدخل المعرف (مثلاً رقم اللاعب)" 
                            className="text-right h-11 bg-muted/50 border-none rounded-xl" 
                        ...
                        <div className="flex items-center justify-between pt-3 border-t border-dashed">
                          <div className="text-right">
                             <p className="text-[10px] text-muted-foreground font-bold">التكلفة الإجمالية</p>
                          ...
                          <Button 
                            ...
                            disabled={ordering === String(product.id)}
                            className="rounded-2xl bg-primary font-bold px-8 h-12 shadow-lg shadow-primary/20 active:scale-95 transition-all"
                          >
                            {ordering === String(product.id) ? <Loader2 className="h-4 w-4 animate-spin" /> : "شحن الآن"}
                          </Button>
                        </div>
                      </CardContent>
                    </Card>
                ...
                <div className="flex flex-col items-center justify-center py-20 text-muted-foreground gap-4 bg-white/50 rounded-3xl mx-2 mt-4 border border-dashed">
                  ...
                  <p className="text-sm font-bold text-foreground">عذراً، لا توجد منتجات متاحة لهذه الفئة حالياً.</p>
                </div>
              )}
            </ScrollArea>
          )}
